...
export default function TabLayout() {
  ...
  useEffect(() => {
    getUserInfo().then((u) => setRole(u?.role ?? null));
  }, []);

  return (
    <Tabs screenOptions={{ headerShown: true, tabBarActiveTintColor: '#007AFF' }}>
      <Tabs.Screen name="home" options={{ title: 'Home' }} />
      {role !== 'driver' && <Tabs.Screen name="ride-booking" options={{ title: 'Book Ride' }} />}
      {role !== 'driver' && <Tabs.Screen name="ride-history" options={{ title: 'Ride History' }} />}
